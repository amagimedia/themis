    ...
    def invalid_properties(self, properties_to_validate):
        ...
        for ip_permission in ip_permissions:
            ...
            for cidrs in ip_ranges:
                ...
                if "0.0.0.0/0" == cidr:
                    if helper.get_from_dict(ip_permission, 'ipProtocol', "IpProtocol") == '-1':
                        if not from_port:
                            from_port = 0
                        if not to_port:
                            to_port = 65535
                    ...
                    for allowed_port_range in allowed_port_ranges:
                        allowed = (allowed_port_range["from"] <= from_port and to_port <= allowed_port_range["to"])
                        if allowed:
                            break
...
class BadIngressEvaluator(ThemisEvaluator):
    ...
    def applicable(self):
        try:
            ...
            if self.event.diff() is not None and self.event.is_update_event():
                for changed_property_name in self.event.diff().get('changedProperties', {}).keys():
    ...
    def handle(self):
        ip_permissions = self.event.configuration_item()["configuration"]["ipPermissions"]
...
class BadIngressFixer(ThemisFixer):
    ...
    def handle(self):
        invalid_ip_permissions = self.rule.invalid_properties(
            self.ip_permissions()
        )
